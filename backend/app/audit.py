...
def _normalize_entity_snapshot(
    *,
    ...
    if entity != "transaction":
        # Basic datetime formatting was already applied in _snapshot_from_changes().
        return snapshot
    ...
    # `user_id` isn't always in the `changes` dict (e.g. normal user tx create), but is useful.
    ...
    # Append any other keys (future-proof)
    for k, v in snapshot.items():
        if k in {"type", "amount", "currency", "note", "occurred_at", "is_voided", "user_id", "category_ids", "tag_ids"}:
            continue
        ordered[k] = v
...
def audit_log(
    *,
    ...
    # Normalize admin action names to the common patterns used below.
    # Examples:
    # - admin.user_create        -> user.create
    # - admin.transaction_bulk_delete -> transaction.bulk_delete
    ...
    if action.startswith("admin."):
        ...
        # Example: user.me_update -> user.me.update
        for suffix, repl in (
            ("_create", ".create"),
            ("_update", ".update"),
            ("_delete", ".delete"),
    ...
    if normalized_action == "auth.login_failed":
        ...
        line = f"[{ts} - {ip}] {actor_text} Login failed{tail}"
        _write_prefixed_line(prefix="audit", line=line)
        return
    ...
    if ".bulk_" in normalized_action:
        ...
        line = f"[{ts} - {ip}] {actor_text} {verb} {label} { _json_compact(ids) }"
        _write_prefixed_line(prefix="audit", line=line)
        return

    # Create / Update / Delete (default patterns)
    ...
    # Admin actions (user/transaction) that don't end with CRUD suffix
    if action == "admin.user_reset_password":
        line = f"[{ts} - {ip}] {actor_text} Reset password user ({entity_id})"
        _write_prefixed_line(prefix="audit", line=line)
        return

    # Fallback: keep one-line, but include structured tail for debugging.
...
def error_log(
    *,
    ...
    line = f"[{ts} - {ip}] ERROR {_json_compact(payload)}"
    _write_prefixed_line(prefix="error", line=line)
